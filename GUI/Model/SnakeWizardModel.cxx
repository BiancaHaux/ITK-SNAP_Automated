...
{
	std::ofstream ofs;
	ofs.open("consoleOutput.txt", std::ofstream::out | std::ofstream::app);
	ofs << "Entered GetEdgePreSigmaValueAndRange() in SnakeWizardModel.cxx\n";
  ...
  if (range){
	  ofs << "Set range to Set(0.1, 3, 0.01)\n";
...
::SetEdgePreprocessingSigmaValue(double x)
{
	std::ofstream ofs;
	ofs.open("consoleOutput.txt", std::ofstream::out | std::ofstream::app);
	ofs << "Entered SetEdgePreprocessingSigmaValue() in SnakeWizardModel.cxx\n";
	ofs << "SetGaussianBlurScale to x = " << x << "\n";
	ofs.close();
    ...
    double &x, NumericValueRange<double> *range)
{
	std::ofstream ofs;
	ofs.open("consoleOutput.txt", std::ofstream::out | std::ofstream::app);
	ofs << "Entered GetEdgePreprocessingKappaValueAndRange() in SnakeWizardModel.cxx\n";
  ...
  if (range){
	  ofs << "Set Range to Set(0.001, 0.2, 0.001)\n";
...
::SetEdgePreprocessingKappaValue(double x)
{
	std::ofstream ofs;
	ofs.open("consoleOutput.txt", std::ofstream::out | std::ofstream::app);
	ofs << "Entered SetEdgePreprocessingKappaValue() in SnakeWizardModel.cxx\n";
	ofs << "SetRemappingSteepness to x = " << x << "\n";
	ofs.close();
    ...
    double &x, NumericValueRange<double> *range)
{
	std::ofstream ofs;
	ofs.open("consoleOutput.txt", std::ofstream::out | std::ofstream::app);
	ofs << "Entered GetEdgePreprocessingExponentValueAndRange() in SnakeWizardModel.cxx\n";
...
::SetEdgePreprocessingExponentValue(double x)
{
	std::ofstream ofs;
	ofs.open("consoleOutput.txt", std::ofstream::out | std::ofstream::app);
	ofs << "Entered SetEdgePreprocessingExponentValue() in SnakeWizardModel.cxx\n";
	ofs << "SetRemappingExponent to x = " << x << "\n";
	ofs.close();
...
void SnakeWizardModel::ApplyPreprocessing()
{
	std::ofstream ofs;
	ofs.open("consoleOutput.txt", std::ofstream::out | std::ofstream::app);
	ofs << "Entered ApplyPreprocessing() in SnakeWizardModel.cxx\n";
	ofs.close();
...
void SnakeWizardModel::CompletePreprocessing()
{
	std::ofstream ofs;
	ofs.open("consoleOutput.txt", std::ofstream::out | std::ofstream::app);
	ofs << "Entered CompletePreprocessing() in SnakeWizardModel.cxx\n";
	ofs.close();
...
bool SnakeWizardModel::GetNumberOfGMMSamplesValueAndRange(int &value, NumericValueRange<int> *range)
{
  ...
  if(uc)
    ...
    if(range)
      ...
      range->Set(std::min(nvox, 1), nvox, 1);
      }
