...
int paramTesting(string paramFilePath, GlobalUIModel *gui){

	pugi::xml_document doc;
	doc.load_file(paramFilePath.c_str());
	xml_node root = doc.document_element();


	//for (xml_node node = root.child("method"); node; node = node.next_sibling("method"))
	//{

		//Define snakeparameters
		xml_node nodeSnake = root.child("snake");
		string snakeType = nodeSnake.attribute("type").as_string();
		SnakeParameters snakeParameters;

		if (snakeType.compare("In-Out") == 0){
			float propagationWeight = nodeSnake.child("propagationWeight").text().as_float();
			int propagationExponent = nodeSnake.child("propagationExponent").text().as_int();
			float curvatureWeight = nodeSnake.child("curvatureWeight").text().as_float();
			int curvatureSpeedExponent = nodeSnake.child("curvatureExponent").text().as_int();

			snakeParameters = SnakeParameters::GetDefaultInOutParameters();
			snakeParameters.SetPropagationWeight(propagationWeight);
			snakeParameters.SetPropagationSpeedExponent(propagationExponent);
			snakeParameters.SetCurvatureWeight(curvatureWeight);
			snakeParameters.SetCurvatureSpeedExponent(curvatureSpeedExponent);
		}

		else if (snakeType.compare("Edge") == 0){
			float propagationWeight = nodeSnake.child("propagationWeight").text().as_float();
			int propagationExponent = nodeSnake.child("propagationExponent").text().as_int();
			float curvatureWeight = nodeSnake.child("curvatureWeight").text().as_float();
			int curvatureSpeedExponent = nodeSnake.child("curvatureExponent").text().as_int();
			float advectionWeight = nodeSnake.child("advectionWeight").text().as_float();
			int advectionSpeedExponent = nodeSnake.child("advectionExponent").text().as_int();

			snakeParameters = SnakeParameters::GetDefaultEdgeParameters();
			snakeParameters.SetPropagationWeight(propagationWeight);
			snakeParameters.SetPropagationSpeedExponent(propagationExponent);
			snakeParameters.SetCurvatureWeight(curvatureWeight);
			snakeParameters.SetCurvatureSpeedExponent(curvatureSpeedExponent);
			snakeParameters.SetAdvectionWeight(advectionWeight);
			snakeParameters.SetAdvectionSpeedExponent(advectionSpeedExponent);
		}



		//Define method parameters
		xml_node nodeMethod = root.child("method");
		string methodName = nodeMethod.attribute("name").as_string();

		if (methodName.compare("Edge") == 0){
			string mainFilePath = nodeMethod.child("mainFile").text().as_string();
			float gaussianBlurScale = nodeMethod.child("gaussianBlurScale").text().as_float();
			float remappingExponent = nodeMethod.child("remappingExponent").text().as_float();
			float remappingSteepness = nodeMethod.child("remappingSteepness").text().as_float();
			double imageNumber = nodeMethod.child("id").text().as_double();

			const char *mainPath = mainFilePath.c_str();
			edgeTest(gui, mainPath, gaussianBlurScale, remappingExponent, remappingSteepness, imageNumber, snakeParameters);
		}
		else if (methodName.compare("Threshold") == 0){
			string mainFilePath = nodeMethod.child("mainFile").text().as_string();
			int lowerThreshold = nodeMethod.child("lowerThreshold").text().as_int();
			int upperThreshold = nodeMethod.child("upperThreshold").text().as_int();
			float smoothness = nodeMethod.child("smoothness").text().as_float();
			double imageNumber = nodeMethod.child("id").text().as_double();

			const char *mainPath = mainFilePath.c_str();
			thresholdTest(gui, mainPath, lowerThreshold, upperThreshold, smoothness, imageNumber, snakeParameters);
		}
		else if (methodName.compare("Clustering") == 0){
			string mainFilePath = nodeMethod.child("mainFile").text().as_string();
			int numberOfClusters = nodeMethod.child("numberOfClusters").text().as_int();
			int numberOfGMMSamples = nodeMethod.child("numberOfGMMSamples").text().as_int();
			int foregroundCluster = nodeMethod.child("foregroundCluster").text().as_int();
			double imageNumber = nodeMethod.child("id").text().as_double();

			const char *mainPath = mainFilePath.c_str();
			clusteringTest(gui, mainPath, numberOfClusters, numberOfGMMSamples, foregroundCluster, imageNumber, snakeParameters);
		}
		else if (methodName.compare("RandomForest") == 0){
			string mainFilePath = nodeMethod.child("mainFile").text().as_string();
			string labelFilePath = nodeMethod.child("labelFile").text().as_string();
			int forestSize = nodeMethod.child("forestSize").text().as_int();
			int treeDepth = nodeMethod.child("treeDepth").text().as_int();
			bool classifierUseCoordinates = nodeMethod.child("classifierUseCoordinates").text().as_bool();
			bool classifierUsePatch = nodeMethod.child("classifierUsePatch").text().as_bool();
			int foregroundLabel = nodeMethod.child("foregroundLabel").text().as_int();
			float bias = nodeMethod.child("bias").text().as_float();
			double imageNumber = nodeMethod.child("id").text().as_double();

			const char *mainPath = mainFilePath.c_str();
			const char *labelPath = labelFilePath.c_str();
			randomForestTest(gui, mainPath, labelPath, forestSize, treeDepth, classifierUseCoordinates, classifierUsePatch, foregroundLabel, bias, imageNumber, snakeParameters);
		//}
	}
...
{
	ofstream myfile("consoleOutput.txt");
	
	int const numOfArguments = argc;
  ...
  try 
	...
	//edgeTest(gui, mainFilePath);
	//thresholdTest(gui, mainFilePath);
	//clusteringTest(gui, mainFilePath);
	//randomForestTest(gui, mainFilePath, labelFilePath);

	paramTesting(paramFilePath, gui);

	//paramTesting("C:/Uni/ParamFiles/Threshold/XML/thresholdParams26.xml", gui);
    ...
    std::cerr << "Return code : " << 0 << std::endl;
    return 0;
    }
  catch(std::exception &exc)
    {
