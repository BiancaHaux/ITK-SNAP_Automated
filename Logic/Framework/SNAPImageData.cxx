...
::InitializeSpeed()
{
	...
	ofs << "Entered InitializeSpeed() in SNAPImageData.cxx\n";
	ofs.close();
...
::GetSpeed() 
{
	std::ofstream ofs;
	ofs.open("consoleOutput.txt", std::ofstream::out | std::ofstream::app);
	ofs << "Entered GetSpeed() in SNAPImageData.cxx\n";
	ofs.close();

	
	
	//SaveImageToFile("speedEdge.nii.gz", m_SpeedWrapper.GetPointer()->GetImage());
	//bool isScalar = m_SpeedWrapper.GetPointer()->IsScalar();
