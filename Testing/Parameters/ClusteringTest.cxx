...
int clusteringTest(GlobalUIModel *gui, const char *mainFilePath, int numberOfClusters, int numberOfGMMSamples, int foregroundCluster, double imageNumber, SnakeParameters snakeParams)
{
	...
	**/
	...
	driver->EnterPreprocessingMode(PREPROCESS_GMM);

	/**
	*Specify Clustering settings.
	**/
	gui->GetSnakeWizardModel()->SetNumberOfClusters(numberOfClusters);
	gui->GetSnakeWizardModel()->SetNumberOfGMMSamples(numberOfGMMSamples);
	gui->GetSnakeWizardModel()->SetForegroundCluster(foregroundCluster);
	//gui->GetSnakeWizardModel()->SetClusterWeight(0, 0.33);
	//gui->GetSnakeWizardModel()->SetClusterWeight(1, 0.33);
	//gui->GetSnakeWizardModel()->SetClusterWeight(2, 0.33);
	//gui->GetSnakeWizardModel()->SetClusterNativeMean(0, 0, -1017);
	//gui->GetSnakeWizardModel()->SetClusterNativeMean(1, 0, 136.06);
	//gui->GetSnakeWizardModel()->SetClusterNativeMean(2, 0, -768.08);

	/**
	*Train clustering classifier.
	**/
	for (int it = 0; it < 10; it++){
		gui->GetSnakeWizardModel()->PerformClusteringIteration();
	}
	/**
	*Do preprocessing.
	**/
	...
	**/
	stringstream str;
	//str << "X:/Repository/build/debug/SNAP_Results/ClusteringResults/speedClustering" << imageNumber << ".nii.gz";
	str << "C:/Uni/SNAP_Results/ClusteringResults/speedClustering" << imageNumber << ".nii.gz";
	string st = str.str();
	const char* pathSpeed = st.c_str();

	SpeedImageWrapper *wrapSpeed = snapData->GetSpeed();
	//SaveImageToFile(pathSpeed, wrapSpeed->GetImage());
	...
	**/
	...
	//stringStream << "X:/Repository/build/debug/SNAP_Results/ClusteringResults/clusteringResult" << imageNumber << ".nii.gz";
	stringStream << "C:/Uni/SNAP_Results/ClusteringResults/clusteringResult" << imageNumber << ".nii.gz";
