...
int edgeTest(GlobalUIModel *gui, const char *mainFilePath, float gaussianBlurScale, float remappingExponent, float remappingSteepness, double imageNumber, SnakeParameters snakeParams)
{
	...
	**/
	...
	//driver->LoadImage(sImageFileName, MAIN_ROLE, warnings);
	...
	**/
	driver->GetGlobalState()->SetSnakeType(EDGE_SNAKE);
	driver->SetSnakeMode(EDGE_SNAKE);
	driver->EnterPreprocessingMode(PREPROCESS_EDGE);


	/**
	*Specify Edge settings.
	**/
	EdgePreprocessingSettings *edgeSettings = driver->GetEdgePreprocessingSettings();
	edgeSettings->SetGaussianBlurScale(gaussianBlurScale);
	edgeSettings->SetRemappingExponent(remappingExponent);
	edgeSettings->SetRemappingSteepness(remappingSteepness);
	...
	**/
	//SnakeParameters parameters = SnakeParameters::GetDefaultInOutParameters();
	...
	**/
	...
	str << "C:/Uni/SNAP_Results/EdgeResults/speedEdge" << imageNumber << ".nii.gz";
	string st = str.str();
	...
	**/
	...
	//stringStream << "X:/Repository/build/debug/SNAP_Results/EdgeResults/edgeResult" << imageNumber << ".nii.gz";
	stringStream << "C:/Uni/SNAP_Results/EdgeResults/edgeResult" << imageNumber << ".nii.gz";
